...
<<cache = TRUE, echo = FALSE, warnings = FALSE, errors = FALSE>>=
## Obtained from the UN
options(warn=-1)
data <- read.csv("WPP2010_DB4_F2A_POPULATION_BY_AGE_MALE_ANNUAL_1950-2010.csv", header = TRUE)
colnames(data)[3] <- c("region")
colnames(data)[6] <- c("year")
colnames(data)[7:17] <- paste("age", 0:10 * 5, sep = "")
foo <- data[, c(3, 6, 11, 12)]

## Taiwan, Japan, Philippines, and South Korea added in 2000
## Brazil added in 2010
## Latin/South American countries added in 1950
############################################
## Need pre-1950 Canadian demographic data
############################################
countries <- c("Mexico", "Aruba", "Panama", "Columbia", "Cuba", "Honduras", 
  ...
  "United States Virgin Islands", "United States of America", "Canada")
bar <- foo[foo[, 1] %in% countries, ]
bar[, 3] <- as.numeric(levels(bar[, 3]))[bar[, 3]]
bar[, 4] <- as.numeric(levels(bar[, 4]))[bar[, 4]]
bar[, 3:4] <- bar[, 3:4] / 1000

baz <- bar[bar[, 2] %in% c("1960", "1970", "1980", "1990", "2000", "2010"), ]
baz[, 2] <- factor(baz[, 2])
foobar <- split(baz, f = as.factor(baz[, 2]))
pops <- unlist(lapply(foobar, function(x) sum(x[, 3:4])))
dataTaiwan2000 <- 1.56 ## approximation from CIA World Factbook
dataTaiwan2010 <- 1.56 ## approximation from CIA World Factbook
pops[5] <- pops[5] + dataTaiwan2000
pops[6] <- pops[6] + dataTaiwan2010

## estimates for the age 20-29 Canadian population from 1881-1951
#  we estimate this population for 1911-1951 by first computing the proportion 
#  of males that are 20-29 from 1881-1901
Can1881 <- 0.21 + 0.17  # https://www65.statcan.gc.ca/acyb02/1907/acyb02_1907001701a-eng.htm
Can1891 <- 0.24 + 0.19  # https://www65.statcan.gc.ca/acyb02/1907/acyb02_1907001701a-eng.htm
Can1901 <- 0.26 + 0.22  # https://www65.statcan.gc.ca/acyb02/1907/acyb02_1907001701a-eng.htm
Can1911 <- 3.82 * 0.182  # https://www65.statcan.gc.ca/acyb02/1947/acyb02_19470113009-eng.htm
Can1921 <- 4.53 * 0.182 # https://www65.statcan.gc.ca/acyb02/1947/acyb02_19470113009-eng.htm
Can1931 <- 5.37 * 0.182 # https://www65.statcan.gc.ca/acyb02/1947/acyb02_19470113009-eng.htm
Can1941 <- 5.90 * 0.182 # https://www65.statcan.gc.ca/acyb02/1947/acyb02_19470113009-eng.htm
Can1951 <- 7.09 * 0.182 # https://www65.statcan.gc.ca/acyb02/1967/acyb02_19670194010-eng.htm
CanM1881 <- 2.19  # https://www65.statcan.gc.ca/acyb02/1947/acyb02_19470113009-eng.htm
CanM1891 <- 2.16  # https://www65.statcan.gc.ca/acyb02/1947/acyb02_19470113009-eng.htm
CanM1901 <- 2.75  # https://www65.statcan.gc.ca/acyb02/1947/acyb02_19470113009-eng.htm


US1880 <- 2.22 + 1.84  # https://www2.census.gov/library/publications/decennial/1880/vol-01-population/1880_v1-15.pdf
US1900 <- 2.73 + 2.37  # https://www2.census.gov/library/publications/decennial/1900/volume-2/volume-2-p5.pdf
US1890 <- mean(c(US1880,US1900))
US1910 <- 4.07 + 3.79  # https://www2.census.gov/library/publications/decennial/1910/volume-1/volume-1-p6.pdf
US1920 <- 4.02 + 4.09  # https://www2.census.gov/library/publications/decennial/1920/volume-2/41084484v2ch03.pdf
US1930 <- 4.69 + 4.25  # https://www2.census.gov/library/publications/decennial/1930/population-volume-2/16440598v2ch11.pdf
US1940 <- 1.08 + 1.06 + 1.01 + 1.00 + 1.01 + 
  1.01 + 0.99 + 0.98 + 0.97 + 0.95 # https://www2.census.gov/library/publications/decennial/1940/population-volume-4/33973538v4p1ch1.pdf
US1950 <- 5.00 + 5.30 # https://www2.census.gov/library/publications/decennial/1950/population-volume-2/21983999v2p1ch2.pdf
Brazil2010 <- 8.62 # age 20-24, year 2010
Japan <- c(sum(c(4.29, 4.96)), sum(c(3.37, 3.84))) # age 20-29, years 2000-2010
Korea <- c(sum(c(2.03, 2.06)), sum(c(1.62, 1.91))) # age 20-29, years 2000-2010
Philippines <- c(sum(c(3.72, 3.15)),sum(c(4.51, 3.92))) # age 20-29, years 2000-2010
aspop <- Japan + Korea + Philippines
...
pops <- as.numeric(c(US1880 + Can1881, 
  ...
  US1950 + Can1951, 
  pops))
pops[14] <- pops[14] + Brazil2010 
pops[13:14] <- pops[13:14] + aspop
...
dataset, consider the year 1950. There were \Sexpr{round(data[8, 2], 3)} 
million males aged 20-29. The proportion of the historical MLB eligible 
population that existed at or before 1950 is \Sexpr{round(data[8, 3], 3)}. 
...
# the dataset
library(xtable)
colnames(data)[c(2:3)] <- c("population", "cumulative population proportion")
print(xtable(as.data.frame(data), digits = c(0,0,3,3), 
  align = c("l","c","c","c"), 
...
before 1950 was approximately \Sexpr{round(data[8,3], 3)}.  
...
observing each individual is about \Sexpr{round(data[8,3], 3)}.  We calculate 
...
<<echo = FALSE>>=
pranker10 <- pbinom(ranker10 - 1, p = p, size = n10, lower = FALSE)
pbWAR10   <- pbinom(bWAR10 - 1, p = p, size = n10, lower = FALSE)
pfWAR10   <- pbinom(fWAR10 - 1, p = p, size = n10, lower = FALSE)
pESPN10   <- pbinom(ESPN10 - 1, p = p, size = n10, lower = FALSE)

pranker25 <- pbinom(ranker25 - 1, p = p, size = n25, lower = FALSE)
pbWAR25   <- pbinom(bWAR25 - 1, p = p, size = n25, lower = FALSE)
pfWAR25   <- pbinom(fWAR25 - 1, p = p, size = n25, lower = FALSE)
pESPN25   <- pbinom(ESPN25 - 1, p = p, size = n25, lower = FALSE)
@
...
probability of extreme event in top 10 list 
  & \Sexpr{signif(pranker10, 3)} 
  & \Sexpr{signif(pbWAR10, 3)} 
  & \Sexpr{signif(pfWAR10, 3)} 
  & \Sexpr{signif(pESPN10, 3)} \\
probability of extreme event in top 25 list 
  & \Sexpr{signif(pranker25, 3)} 
  & \Sexpr{signif(pbWAR25, 3)} 
  & \Sexpr{signif(pfWAR25, 3)} 
  & \Sexpr{signif(pESPN25, 3)} \\
chance of extreme event in top 10 list 
  & 1 in \Sexpr{round(1/pranker10)} 
  & 1 in \Sexpr{round(1/pbWAR10)} 
  & 1 in \Sexpr{round(1/pfWAR10)} 
  & 1 in \Sexpr{round(1/pESPN10)} \\
chance of extreme event in top 25 list 
  & 1 in \Sexpr{round(1/pranker25)} 
  & 1 in \Sexpr{round(1/pbWAR25)} 
  & 1 in \Sexpr{round(1/pfWAR25)} 
  & 1 in \Sexpr{round(1/pESPN25)} \\
  \hline
\end{tabular}
\end{center}
...
dynamics, is about \Sexpr{signif(pbWAR10, 3)} 
(a chance of 1 in \Sexpr{round(1/pbWAR10)}).
...
# Information from Gallup on baseball fans (50% fans before 1940)
Fan1880 <- Fan1890 <- Fan1900 <- Fan1910 <- Fan1920 <- Fan1930 <-  0.50
Fan1940 <- Fan1950 <- Fan1960 <- Fan1970 <- Fan1980 <- Fan1990 <- 
  Fan2000 <- Fan2010 <- Fan2015 <- 0.40
w1 <- c(Fan1880, Fan1890, Fan1900, Fan1910, Fan1920, Fan1930, Fan1940, 
  Fan1950, Fan1960, Fan1970, Fan1980, Fan1990, Fan2000, Fan2010, Fan2015)

data.w1 <- data
data.w1[, 2] <- data.w1[, 2] * w1
data.w1[, 3] <- cumsum(data.w1[, 2]) / sum(data.w1[, 2])


# Information from Gallup on baseball fans (60% fans before 1940)
Fan1880 <- Fan1890 <- Fan1900 <- Fan1910 <- Fan1920 <- Fan1930 <- 0.60
w2 <- c(Fan1880, Fan1890, Fan1900, Fan1910, Fan1920, Fan1930, Fan1940, 
	Fan1950, Fan1960, Fan1970, Fan1980, Fan1990, Fan2000, Fan2010, Fan2015) 

data.w2 <- data
data.w2[, 2] <- data.w2[, 2] * w2
data.w2[, 3] <- cumsum(data.w2[, 2]) / sum(data.w2[, 2])


# Information from Gallup on favorite sport
Fan1880 <- Fan1890 <- Fan1900 <- Fan1910 <- Fan1920 <- 0.40
Fan1930 <- 0.40
Fan1940 <- 0.35; Fan1950 <- 0.38; Fan1960 <- 0.34; Fan1970 <- 0.28
Fan1980 <- 0.16; Fan1990 <- 0.16; Fan2000 <- 0.13; Fan2010 <- 0.12
Fan2015 <- 0.10
w3 <- c(Fan1880, Fan1890, Fan1900, Fan1910, Fan1920, Fan1930, Fan1940, 
  Fan1950, Fan1960, Fan1970, Fan1980, Fan1990, Fan2000, Fan2010, Fan2015)

data.w3 <- data
data.w3[, 2] <- data.w3[, 2] * w3
data.w3[, 3] <- cumsum(data.w3[, 2]) / sum(data.w3[, 2])
@
...
<<echo = FALSE>>=
## Recompute probabilities consistent with w1
p.w1 <- data.w1[8, 3]

pranker10.w1 <- pbinom(ranker10 - 1, p = p.w1, size = n10, lower = FALSE)
pbWAR10.w1   <- pbinom(bWAR10 - 1, p = p.w1, size = n10, lower = FALSE)
pfWAR10.w1   <- pbinom(fWAR10 - 1, p = p.w1, size = n10, lower = FALSE)
pESPN10.w1   <- pbinom(ESPN10 - 1, p = p.w1, size = n10, lower = FALSE)

pranker25.w1 <- pbinom(ranker25 - 1, p = p.w1, size = n25, lower = FALSE)
pbWAR25.w1   <- pbinom(bWAR25 - 1, p = p.w1, size = n25, lower = FALSE)
pfWAR25.w1   <- pbinom(fWAR25 - 1, p = p.w1, size = n25, lower = FALSE)
pESPN25.w1   <- pbinom(ESPN25 - 1, p = p.w1, size = n25, lower = FALSE)

## Recompute probabilities consistent with w2
p.w2 <- data.w2[8, 3]

pranker10.w2 <- pbinom(ranker10 - 1, p = p.w2, size = n10, lower = FALSE)
pbWAR10.w2   <- pbinom(bWAR10 - 1, p = p.w2, size = n10, lower = FALSE)
pfWAR10.w2   <- pbinom(fWAR10 - 1, p = p.w2, size = n10, lower = FALSE)
pESPN10.w2   <- pbinom(ESPN10 - 1, p = p.w2, size = n10, lower = FALSE)

pranker25.w2 <- pbinom(ranker25 - 1, p = p.w2, size = n25, lower = FALSE)
pbWAR25.w2   <- pbinom(bWAR25 - 1, p = p.w2, size = n25, lower = FALSE)
pfWAR25.w2   <- pbinom(fWAR25 - 1, p = p.w2, size = n25, lower = FALSE)
pESPN25.w2   <- pbinom(ESPN25 - 1, p = p.w2, size = n25, lower = FALSE)

## Recompute probabilities consistent with w3
p.w3 <- data.w3[8, 3]

pranker10.w3 <- pbinom(ranker10 - 1, p = p.w3, size = n10, lower = FALSE)
pbWAR10.w3   <- pbinom(bWAR10 - 1, p = p.w3, size = n10, lower = FALSE)
pfWAR10.w3   <- pbinom(fWAR10 - 1, p = p.w3, size = n10, lower = FALSE)
pESPN10.w3   <- pbinom(ESPN10 - 1, p = p.w3, size = n10, lower = FALSE)

pranker25.w3 <- pbinom(ranker25 - 1, p = p.w3, size = n25, lower = FALSE)
pbWAR25.w3   <- pbinom(bWAR25 - 1, p = p.w3, size = n25, lower = FALSE)
pfWAR25.w3   <- pbinom(fWAR25 - 1, p = p.w3, size = n25, lower = FALSE)
pESPN25.w3   <- pbinom(ESPN25 - 1, p = p.w3, size = n25, lower = FALSE)

## Recompute probabilities consistent with w4
w4 <- (w2 + w3) / 2
data.w4 <- data
data.w4[, 2] <- data.w4[, 2] * w4
data.w4[, 3] <- cumsum(data.w4[, 2]) / sum(data.w4[, 2])
p.w4 <- data.w4[8, 3]

pranker10.w4 <- pbinom(ranker10 - 1, p = p.w4, size = n10, lower = FALSE)
pbWAR10.w4   <- pbinom(bWAR10 - 1, p = p.w4, size = n10, lower = FALSE)
pfWAR10.w4   <- pbinom(fWAR10 - 1, p = p.w4, size = n10, lower = FALSE)
pESPN10.w4   <- pbinom(ESPN10 - 1, p = p.w4, size = n10, lower = FALSE)

pranker25.w4 <- pbinom(ranker25 - 1, p = p.w4, size = n25, lower = FALSE)
pbWAR25.w4   <- pbinom(bWAR25 - 1, p = p.w4, size = n25, lower = FALSE)
pfWAR25.w4   <- pbinom(fWAR25 - 1, p = p.w4, size = n25, lower = FALSE)
pESPN25.w4   <- pbinom(ESPN25 - 1, p = p.w4, size = n25, lower = FALSE)
@


<<echo = FALSE, results = "asis">>=
tab <- cbind(w1,w2,w3,w4)
rownames(tab) <- year
#colnames(tab)[1] <- c("year")
#tab <- as.data.frame(tab)
#rownames(tab) <- NULL
print(xtable(t(tab), #digits = c(0,0,2,2,2,2), 
  digits = cbind(rep(0, 4), matrix(2, ncol = 15, nrow = 4)),
  caption = "Weighting regimes."), size = "footnotesize",
...
w5 <- c(0.20, 0.20, 0.20, 0.20, 0.25, 0.40, 0.40, 0.35, 0.38, 
  0.34, 0.28, 0.16, 0.13, 0.12, 0.20)
data.w5 <- data
data.w5[, 2] <- data.w5[, 2] * w5
data.w5[, 3] <- cumsum(data.w5[, 2]) / sum(data.w5[, 2])
p.w5 <- data.w5[8, 3]
...
pranker10.w5 <- pbinom(ranker10 - 1, p = p.w5, size = n10, lower = FALSE)
pbWAR10.w5   <- pbinom(bWAR10 - 1, p = p.w5, size = n10, lower = FALSE)
pfWAR10.w5   <- pbinom(fWAR10 - 1, p = p.w5, size = n10, lower = FALSE)
pESPN10.w5   <- pbinom(ESPN10 - 1, p = p.w5, size = n10, lower = FALSE)

pranker25.w5 <- pbinom(ranker25 - 1, p = p.w5, size = n25, lower = FALSE)
pbWAR25.w5   <- pbinom(bWAR25 - 1, p = p.w5, size = n25, lower = FALSE)
pfWAR25.w5   <- pbinom(fWAR25 - 1, p = p.w5, size = n25, lower = FALSE)
pESPN25.w5   <- pbinom(ESPN25 - 1, p = p.w5, size = n25, lower = FALSE)

c(pranker10.w5, pbWAR10.w5, pfWAR10.w5, pESPN10.w5)
1 / c(pranker10.w5, pbWAR10.w5, pfWAR10.w5, pESPN10.w5)
c(pranker25.w5, pbWAR25.w5, pfWAR25.w5, pESPN25.w5)
1 / c(pranker25.w5, pbWAR25.w5, pfWAR25.w5, pESPN25.w5)
@
...
w1 & probability of extreme event in top 10 list 
  & \Sexpr{signif(pranker10.w1, 3)} 
  & \Sexpr{signif(pbWAR10.w1, 3)} 
  & \Sexpr{signif(pfWAR10.w1, 3)} 
  & \Sexpr{signif(pESPN10.w1, 3)} \\
& probability of extreme event in top 25 list 
  & \Sexpr{signif(pranker25.w1, 3)} 
  & \Sexpr{signif(pbWAR25.w1, 3)} 
  & \Sexpr{signif(pfWAR25.w1, 3)} 
  & \Sexpr{signif(pESPN25.w1, 3)} \\
& chance of extreme event in top 10 list 
  & 1 in \Sexpr{round(1/pranker10.w1)} 
  & 1 in \Sexpr{round(1/pbWAR10.w1)} 
  & 1 in \Sexpr{round(1/pfWAR10.w1)} 
  & 1 in \Sexpr{round(1/pESPN10.w1)} \\
& chance of extreme event in top 25 list 
  & 1 in \Sexpr{round(1/pranker25.w1)} 
  & 1 in \Sexpr{round(1/pbWAR25.w1)} 
  & 1 in \Sexpr{round(1/pfWAR25.w1)} 
  & 1 in \Sexpr{round(1/pESPN25.w1)} \\
  & & & & & \\
w2 & probability of extreme event in top 10 list 
  & \Sexpr{signif(pranker10.w2, 3)} 
  & \Sexpr{signif(pbWAR10.w2, 3)} 
  & \Sexpr{signif(pfWAR10.w2, 3)} 
  & \Sexpr{signif(pESPN10.w2, 3)} \\
& probability of extreme event in top 25 list 
  & \Sexpr{signif(pranker25.w2, 3)} 
  & \Sexpr{signif(pbWAR25.w2, 3)} 
  & \Sexpr{signif(pfWAR25.w2, 3)} 
  & \Sexpr{signif(pESPN25.w2, 3)} \\
& chance of extreme event in top 10 list 
  & 1 in \Sexpr{round(1/pranker10.w2)} 
  & 1 in \Sexpr{round(1/pbWAR10.w2)} 
  & 1 in \Sexpr{round(1/pfWAR10.w2)} 
  & 1 in \Sexpr{round(1/pESPN10.w2)} \\
& chance of extreme event in top 25 list 
  & 1 in \Sexpr{round(1/pranker25.w2)} 
  & 1 in \Sexpr{round(1/pbWAR25.w2)} 
  & 1 in \Sexpr{round(1/pfWAR25.w2)} 
  & 1 in \Sexpr{round(1/pESPN25.w2)} \\
  & & & & & \\
w3 & probability of extreme event in top 10 list 
  & \Sexpr{signif(pranker10.w3, 3)} 
  & \Sexpr{signif(pbWAR10.w3, 3)} 
  & \Sexpr{signif(pfWAR10.w3, 3)} 
  & \Sexpr{signif(pESPN10.w3, 3)} \\
& probability of extreme event in top 25 list 
  & \Sexpr{signif(pranker25.w3, 3)} 
  & \Sexpr{signif(pbWAR25.w3, 3)} 
  & \Sexpr{signif(pfWAR25.w3, 3)} 
  & \Sexpr{signif(pESPN25.w3, 3)} \\
& chance of extreme event in top 10 list 
  & 1 in \Sexpr{round(1/pranker10.w3)} 
  & 1 in \Sexpr{round(1/pbWAR10.w3)} 
  & 1 in \Sexpr{round(1/pfWAR10.w3)} 
  & 1 in \Sexpr{round(1/pESPN10.w3, 1)} \\
& chance of extreme event in top 25 list 
  & 1 in \Sexpr{round(1/pranker25.w3)} 
  & 1 in \Sexpr{round(1/pbWAR25.w3)} 
  & 1 in \Sexpr{round(1/pfWAR25.w3, 1)} 
  & 1 in \Sexpr{round(1/pESPN25.w3, 1)} \\
  & & & & & \\  
w4 & probability of extreme event in top 10 list 
  & \Sexpr{signif(pranker10.w4, 3)} 
  & \Sexpr{signif(pbWAR10.w4, 3)} 
  & \Sexpr{signif(pfWAR10.w4, 3)} 
  & \Sexpr{signif(pESPN10.w4, 3)} \\
& probability of extreme event in top 25 list 
  & \Sexpr{signif(pranker25.w4, 3)} 
  & \Sexpr{signif(pbWAR25.w4, 3)} 
  & \Sexpr{signif(pfWAR25.w4, 3)} 
  & \Sexpr{signif(pESPN25.w4, 3)} \\
& chance of extreme event in top 10 list 
  & 1 in \Sexpr{round(1/pranker10.w4)} 
  & 1 in \Sexpr{round(1/pbWAR10.w4)} 
  & 1 in \Sexpr{round(1/pfWAR10.w4)} 
  & 1 in \Sexpr{round(1/pESPN10.w4, 1)} \\
& chance of extreme event in top 25 list 
  & 1 in \Sexpr{round(1/pranker25.w4)} 
  & 1 in \Sexpr{round(1/pbWAR25.w4)} 
  & 1 in \Sexpr{round(1/pfWAR25.w4)} 
  & 1 in \Sexpr{round(1/pESPN25.w4)} \\
  \hline
\end{tabular}
\end{center}
...
\hline
%\Sexpr{data[1,1]} & \Sexpr{data[1,2]}  & 8  & 15 & \Sexpr{round(data[1,2] / (8*15) * 1e3, 1)}   \\
\Sexpr{data[2,1]} & \Sexpr{data[2,2]}  & 8  & 15 & \Sexpr{round(data[2,2] / (8*15) * 1e3, 1)}   \\
%\Sexpr{data[3,1]} & \Sexpr{data[3,2]}  & 8  & 15 & \Sexpr{round(data[3,2] / (8*15) * 1e3, 1)}   \\
\Sexpr{data[4,1]} & \Sexpr{data[4,2]}  & 16 & 25 & \Sexpr{round(data[4,2] / (16*25) * 1e3, 1)}  \\
%\Sexpr{data[5,1]} & \Sexpr{data[5,2]}  & 16 & 25 & \Sexpr{round(data[5,2] / (16*25) * 1e3, 1)}  \\
  %& & & &  \\
\Sexpr{data[6,1]} & \Sexpr{data[6,2]}  & 16 & 25 & \Sexpr{round(data[6,2] / (16*25) * 1e3, 1)}  \\
%\Sexpr{data[7,1]} & \Sexpr{data[7,2]}  & 16 & 25 & \Sexpr{round(data[7,2] / (16*25) * 1e3, 1)}  \\
\Sexpr{data[8,1]} & \Sexpr{data[8,2]}  & 16 & 25 & \Sexpr{round(data[8,2] / (16*25) * 1e3, 1)}  \\
%\Sexpr{data[9,1]} & \Sexpr{data[9,2]}  & 16 & 25 & \Sexpr{round(data[9,2] / (16*25) * 1e3, 1)}  \\
\Sexpr{data[10,1]} & \Sexpr{data[10,2]}  & 24 & 25 & \Sexpr{round(data[10,2] / (24*25) * 1e3, 1)}  \\
  %& & & &  \\
%\Sexpr{data[11,1]} & \Sexpr{data[11,2]} & 26 & 25 & \Sexpr{round(data[11,2] / (26*25) * 1e3, 1)} \\
\Sexpr{data[12,1]} & \Sexpr{data[12,2]} & 26 & 25 & \Sexpr{round(data[12,2] / (26*25) * 1e3, 1)} \\
%\Sexpr{data[13,1]} & \Sexpr{data[13,2]} & 30 & 25 & \Sexpr{round(data[13,2] / (30*25) * 1e3, 1)} \\
\Sexpr{data[14,1]} & \Sexpr{data[14,2]} & 30 & 25 & \Sexpr{round(data[14,2] / (30*25) * 1e3, 1)} \\
\hline
\end{tabular}
\end{center}
...
<<echo = FALSE>>=
# 6 of the 10 best hitters for average started their career before 1950 
# 10 of the 25 best hitters for average started their career before 1950 
data2 <- data[1:13, ]
data2[13, 2] <- 0.9 * data[13, 2]
data2[, 3] <- cumsum(data2[, 2]) / sum(data2[, 2])
p2 <- data2[8, 3]

bridge10 <- 6; bridge25 <- 10
pbridge10 <- pbinom(bridge10 - 1, p = p2, size = n10, lower = FALSE)
pbridge25 <- pbinom(bridge25 - 1, p = p2, size = n25, lower = FALSE)
@
...
before 1950 as 1 in \Sexpr{round(1 / pbridge10)}. 
...
1 in \Sexpr{round(1 / pbridge25, 2)}.  These chances are not as extreme 
...
%\hline
%\Sexpr{data[1,1]} & \Sexpr{round(data[1,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w1[1,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w2[1,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w3[1,2] / (15*8) * 1e3, 1)} 
%  & \Sexpr{round(data.w4[1,2] / (15*8) * 1e3, 1)}   \\
%\Sexpr{data[2,1]} & \Sexpr{round(data[2,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w1[2,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w2[2,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w3[2,2] / (15*8) * 1e3, 1)} 
%  & \Sexpr{round(data.w4[2,2] / (15*8) * 1e3, 1)}   \\
%\Sexpr{data[3,1]} & \Sexpr{round(data[3,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w1[3,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w2[3,2] / (15*8) * 1e3, 1)}   
%  & \Sexpr{round(data.w3[3,2] / (15*8) * 1e3, 1)} 
%  & \Sexpr{round(data.w4[3,2] / (15*8) * 1e3, 1)}   \\
%\Sexpr{data[4,1]} & \Sexpr{round(data[4,2] / (25*16) * 1e3, 1)}   
%  & \Sexpr{round(data.w1[4,2] / (25*16) * 1e3, 1)}   
%  & \Sexpr{round(data.w2[4,2] / (25*16) * 1e3, 1)}   
%  & \Sexpr{round(data.w3[4,2] / (25*16) * 1e3, 1)} 
%  & \Sexpr{round(data.w4[4,2] / (25*16) * 1e3, 1)}   \\
%\Sexpr{data[5,1]} & \Sexpr{round(data[5,2] / (25*16) * 1e3, 1)}   
%  & \Sexpr{round(data.w1[5,2] / (25*16) * 1e3, 1)}   
%  & \Sexpr{round(data.w2[5,2] / (25*16) * 1e3, 1)}   
%  & \Sexpr{round(data.w3[5,2] / (25*16) * 1e3, 1)} 
%  & \Sexpr{round(data.w4[5,2] / (25*16) * 1e3, 1)}   \\ 
%  %& & & & & \\   
%\Sexpr{data[6,1]} & \Sexpr{round(data[6,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w1[6,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w2[6,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w3[6,2] / (25*16) * 1e3, 1)}
%  & \Sexpr{round(data.w4[6,2] / (25*16) * 1e3, 1)}  \\
%\Sexpr{data[7,1]} & \Sexpr{round(data[7,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w1[7,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w2[7,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w3[7,2] / (25*16) * 1e3, 1)}
%  & \Sexpr{round(data.w4[7,2] / (25*16) * 1e3, 1)}  \\
%\Sexpr{data[8,1]} & \Sexpr{round(data[8,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w1[8,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w2[8,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w3[8,2] / (25*16) * 1e3, 1)}
%  & \Sexpr{round(data.w4[8,2] / (25*16) * 1e3, 1)}  \\
%\Sexpr{data[9,1]} & \Sexpr{round(data[9,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w1[9,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w2[9,2] / (25*16) * 1e3, 1)}  
%  & \Sexpr{round(data.w3[9,2] / (25*16) * 1e3, 1)}
%  & \Sexpr{round(data.w4[9,2] / (25*16) * 1e3, 1)}  \\  
%\Sexpr{data[10,1]} & \Sexpr{round(data[10,2] / (25*24) * 1e3, 1)}  
%  & \Sexpr{round(data.w1[10,2] / (25*24) * 1e3, 1)}  
%  & \Sexpr{round(data.w2[10,2] / (25*24) * 1e3, 1)}  
%  & \Sexpr{round(data.w3[10,2] / (25*24) * 1e3, 1)}
%  & \Sexpr{round(data.w4[10,2] / (25*24) * 1e3, 1)}  \\   
%  %& & & & & \\ 
%\Sexpr{data[11,1]} & \Sexpr{round(data[11,2] / (25*26) * 1e3, 1)} 
%  & \Sexpr{round(data.w1[11,2] / (25*26) * 1e3, 1)} 
%  & \Sexpr{round(data.w2[11,2] / (25*26) * 1e3, 1)} 
%  & \Sexpr{round(data.w3[11,2] / (25*26) * 1e3, 1)}
%  & \Sexpr{round(data.w4[11,2] / (25*26) * 1e3, 1)} \\
%\Sexpr{data[12,1]} & \Sexpr{round(data[12,2] / (25*26) * 1e3, 1)} 
%  & \Sexpr{round(data.w1[12,2] / (25*26) * 1e3, 1)} 
%  & \Sexpr{round(data.w2[12,2] / (25*26) * 1e3, 1)} 
%  & \Sexpr{round(data.w3[12,2] / (25*26) * 1e3, 1)}
%  & \Sexpr{round(data.w4[12,2] / (25*26) * 1e3, 1)} \\  
%\Sexpr{data[13,1]} & \Sexpr{round(data[13,2] / (25*30) * 1e3, 1)} 
%  & \Sexpr{round(data.w1[13,2] / (25*30) * 1e3, 1)} 
%  & \Sexpr{round(data.w2[13,2] / (25*30) * 1e3, 1)} 
%  & \Sexpr{round(data.w3[13,2] / (25*30) * 1e3, 1)}
%  & \Sexpr{round(data.w4[13,2] / (25*30) * 1e3, 1)} \\
%\Sexpr{data[14,1]} & \Sexpr{round(data[14,2] / (25*30) * 1e3, 1)} 
%  & \Sexpr{round(data.w1[14,2] / (25*30) * 1e3, 1)} 
%  & \Sexpr{round(data.w2[14,2] / (25*30) * 1e3, 1)} 
%  & \Sexpr{round(data.w3[14,2] / (25*30) * 1e3, 1)}
%  & \Sexpr{round(data.w4[14,2] / (25*30) * 1e3, 1)} \\
%\hline
%\end{tabular}
%\end{center}
...
%10 occurs with probability $p = \Sexpr{signif(data[8,3], 3)}$ 
...
%$$  
%  P(X = 6) = {10 \choose 6}p^6(1-p)^{4} 
%    = \Sexpr{choose(10, 6)} \times \Sexpr{p}^6 \times (1 - \Sexpr{p})^4
%    = \Sexpr{dbinom(6, size = 10, p = p)} . 
%$$
