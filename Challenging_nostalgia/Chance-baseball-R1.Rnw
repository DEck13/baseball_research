...
<<pops, echo = FALSE, cache = TRUE, warnings = FALSE, errors = FALSE>>=
# load in UN dataset and remove irrelevant variables
options(warn=-1)
WPP2010 <- 
  read.csv("WPP2010_DB4_F2A_POPULATION_BY_AGE_MALE_ANNUAL_1950-2010.csv", 
  header = TRUE)
...
countries <- c("Mexico", "Aruba", "Panama", 
  "Columbia", "Cuba", "Honduras", "Jamaica", 
  "Bahamas", "Peru", "Dominican Republic", 
  "Netherlands Antilles", "Puerto Rico", 
  "Venezuela (Bolivarian Republic of)", "Nicaragua", 
  "United States Virgin Islands", "Canada", 
...
# get population data for relevant countries 
countries <- c("Brazil", "Japan", "Philippines", "Republic of Korea")
dataset <- WPP2010[WPP2010[, 1] %in% countries, ]
dataset[, 3] <- as.numeric(levels(dataset[, 3]))[dataset[, 3]]
dataset[, 4] <- as.numeric(levels(dataset[, 4]))[dataset[, 4]]
dataset[, 3:4] <- dataset[, 3:4] / 1000
...
# get population dataset for this analysis corresponding to the 
# Census years 
dataset.years <- dataset[dataset[, 2] %in% c("2000", "2010"), ]
dataset.years[, 2] <- factor(dataset.years[, 2])
# Brazil age 20-24, year 2010
Brazil2010 <- round(dataset.years[8, 3], 2)
dataset.years <- dataset.years[c(1:3,5:7), ]
dataset.years.list <- split(dataset.years, f = as.factor(dataset.years[, 2]))
aspop <- unlist(lapply(dataset.years.list, function(x) sum(x[, 3:4])))

# refine the population dataset that is used in the analysis
