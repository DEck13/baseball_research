...
<<pops, echo = FALSE, cache = TRUE, warnings = FALSE, errors = FALSE, message = FALSE>>=
...
## countries selected
countries <- c("Aruba", "Bahamas", "Brazil", "Colombia", "Cuba", "Dominican Republic", 
            "Honduras", "Jamaica", "Japan", "Mexico", "Netherlands Antilles", "Nicaragua", 
            "Panama", "Philippines", "Peru", "Puerto Rico", "Republic of Korea", "Taiwan", 
            "Venezuela (Bolivarian Republic of)", "United States Virgin Islands", "Canada", 
...
baz <- lapply(bar, function(xx){
  ...
  if(region == "Brazil"){
    ...
    xx <- xx %>% mutate(total = age20 + age25) %>% dplyr::select("year", "total") %>% filter(year >= 2000)
  }
  else{
    xx <- xx %>% mutate(total = age20 + age25) %>% dplyr::select("year", "total")
  }
...
dataset, consider the year 1950. There were \Sexpr{round(dat[8, 2], 3)} 
million males aged 20-29. The proportion of the historical MLB eligible 
population that existed at or before 1950 is \Sexpr{round(dat[8, 3], 3)}. 
...
# the dataset
library(xtable)
colnames(dat)[c(2:3)] <- c("population", "cumulative population proportion")
print(xtable(as.data.frame(dat), digits = c(0,0,3,3), 
  align = c("l","c","c","c"), 
...
before 1950 was approximately \Sexpr{round(dat[8,3], 3)}.  
...
Several methods are used to compare players across eras by com- puting a 
...
\hline
%\Sexpr{dat[1,1]} & \Sexpr{dat[1,2]}  & 8  & 15 & \Sexpr{round(dat[1,2] / (8*15) * 1e3, 1)}   \\
\Sexpr{dat[2,1]} & \Sexpr{dat[2,2]}  & 8  & 15 & \Sexpr{round(dat[2,2] / (8*15) * 1e3, 1)}   \\
%\Sexpr{dat[3,1]} & \Sexpr{dat[3,2]}  & 8  & 15 & \Sexpr{round(dat[3,2] / (8*15) * 1e3, 1)}   \\
\Sexpr{dat[4,1]} & \Sexpr{dat[4,2]}  & 16 & 25 & \Sexpr{round(dat[4,2] / (16*25) * 1e3, 1)}  \\
%\Sexpr{dat[5,1]} & \Sexpr{dat[5,2]}  & 16 & 25 & \Sexpr{round(dat[5,2] / (16*25) * 1e3, 1)}  \\
  %& & & &  \\
\Sexpr{dat[6,1]} & \Sexpr{dat[6,2]}  & 16 & 25 & \Sexpr{round(dat[6,2] / (16*25) * 1e3, 1)}  \\
%\Sexpr{dat[7,1]} & \Sexpr{dat[7,2]}  & 16 & 25 & \Sexpr{round(dat[7,2] / (16*25) * 1e3, 1)}  \\
\Sexpr{dat[8,1]} & \Sexpr{dat[8,2]}  & 16 & 25 & \Sexpr{round(dat[8,2] / (16*25) * 1e3, 1)}  \\
%\Sexpr{dat[9,1]} & \Sexpr{dat[9,2]}  & 16 & 25 & \Sexpr{round(dat[9,2] / (16*25) * 1e3, 1)}  \\
\Sexpr{dat[10,1]} & \Sexpr{dat[10,2]}  & 24 & 25 & \Sexpr{round(dat[10,2] / (24*25) * 1e3, 1)}  \\
  %& & & &  \\
%\Sexpr{dat[11,1]} & \Sexpr{dat[11,2]} & 26 & 25 & \Sexpr{round(dat[11,2] / (26*25) * 1e3, 1)} \\
\Sexpr{dat[12,1]} & \Sexpr{dat[12,2]} & 26 & 25 & \Sexpr{round(dat[12,2] / (26*25) * 1e3, 1)} \\
%\Sexpr{dat[13,1]} & \Sexpr{dat[13,2]} & 30 & 25 & \Sexpr{round(dat[13,2] / (30*25) * 1e3, 1)} \\
\Sexpr{dat[14,1]} & \Sexpr{dat[14,2]} & 30 & 25 & \Sexpr{round(dat[14,2] / (30*25) * 1e3, 1)} \\
\hline
\end{tabular}
\end{center}
...
list who started their careers before 1950 is 1 in \Sexpr{round(1 / pbridge10)}. 
The chance of expecting to observe 10 or more individuals in a top 25 list who 
started their careers before 1950 is 1 in \Sexpr{round(1 / pbridge25, 2)}.  
