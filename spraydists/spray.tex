...
\end{align*}
We will now choose the bandwidth parameters such that $n_{p,j}\det(\Hbf_{p,j}) = n\det(\Hbf)$ and $n_{b,k}\det(\Hbf_{b,k}) = n\det(\Hbf)$ in order to simplify our derivation for $\lambdabf$. With these specifications, we have that
\begin{align*}
...
$$
  \Var(\hat g_{\lambdabf}(y|x)) - \Var(\hat f_\h(\y|\x)) 
    \approx \left(\lambda^2 + \lambda_p^2 \sum_{j=1}^Jw_{p,j}^2 
      + \lambda_b^2 \sum_{k=1}^K w_{b,k}^2 - 1\right)\frac{R_{\x}(f)f(\y|\x)}{n\det(\Hbf)} 
