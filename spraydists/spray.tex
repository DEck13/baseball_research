...
\begin{abstract}
We develop a methodological and visual framework for assessing pitcher vs batter matchups in baseball. We first estimate the distribution of balls put into play by a batter facing a pitcher, called the spray chart distribution. This distribution is conditional on batter and pitcher characteristics that reflect the true talent of the players under study. Many of these individual matchups have a sample size that is too small to be reliable. Synthetic versions of the batter and pitcher under consideration are constructed in order to alleviate these concerns. The weights that these synthetic players factor into the overall spray chart distribution are developed to minimize expected mean square error. We then provide novel performance metrics that are calculated as expectations taken with respect to the spray chart distribution. These performance metrics provide a context rich approach to player evaluation. Our main contribution is a Shiny app that allows users to evaluate any batter pitcher matchup that has occurred or could have occurred in the last five years. One can access this app here: (URL to Shiny app goes here). This interactive tool has utility for anyone interested in baseball ranging from casual fans to sportswriters to player to team executive.
\end{abstract}
...
\cite{eck2020challenging} showed that context-free metrics and the class of metrics that compares a player's accomplishments directly with that player's peers are ill-equipped for player comparisons across eras of baseball, although they may perform well over the course of a single season or a few consecutive seasons. That being said, these context-free metrics do not offer any guidance for how any particular batter will perform against a particular pitcher, the most interesting outcome in a baseball game. Worse yet is that the baseball outcomes have been assumed to be independent and identically distributed (iid) realizations in the literature \citep{brown2008season, jensen2009hierarchical}. The iid assumption of outcomes may be reasonable in the prediction contexts of \cite{brown2008} and \cite{jensen2009hierarchical} that involve long time frames, but iid is not appropriate for small time frames when batter and pitcher variability can be very large.
...
In this section we present a snapshot of what our Shiny app offers users. The Shiny app is available at: (URL to Shiny app goes here) \\

\noindent Some things to do:
\begin{itemize}
\item[1.] Find one or two interesting matchups. Include pictures of the display \\
The default matchup in the application pairs the reigning American League Cy Young winner against the reigning American League MVP, in Justin Verlander vs. Mike Trout. The layout includes a sidebar with four filters: two dropdowns for pitcher/batter selection and two sliders for metric adjustment.
...
\centering
    ...
    \caption{Mike Trout's traditional spray chart, with no weighting}
\end{figure}

\begin{figure}
\centering
    \includegraphics[width=3in, height=2in]{synthetic_trout.png}
    \caption{Mike Trout's synthetic spray chart vs. Justin Verlander}
\end{figure}

\begin{figure}
\centering
    \includegraphics[width=3in, height=2in]{synthetic_verlander.png}
    \caption{Justin Verlander's synthetic spray chart vs. Mike Trout}
\end{figure}

\begin{figure}
\centering
    \includegraphics[width=3in, height=2in]{trout_verlander_master.png}
    \caption{The complete synthetic spray chart for Mike Trout vs. Justin Verlander}
\end{figure}
...
\textbf{Explanation of sliders}: The slider displays allow users to determine the relative importance of stuff, a colloquial term for pitch quality, versus release point for pitchers and contact quality versus batted ball tendencies for batters in forming the synthetic players. The default setting of the pitcher slider favors stuff over release point information. The logic for this is that quality of pitches is more representative of ability than release point. The default setting of the batter slider favors quality of contact over batted ball tendencies which appears to bias the synthetic batter's spray chart away from that of the batter under consideration. That being said, the batted ball tendencies are recorded as percentages of balls hit to six large grids on the baseball field, ignoring the quality, trajectory, and exact location of the batted ball. Thus, the quality of contact forms a more complete representation of a batter's skill than tendency.



\section{Pitcher and batter characteristics}

The data was acquired via Bill Petti's baseballR package. It is comprised of every pitch thrown since 2015 that has been captured by Statcast. A few preprocessing steps are involved:
...
\begin{itemize}
    ...
    \item Batted ball locations are adjusted to reflect accurate baseball field coordinates (https://www.fangraphs.com/tht/research-notebook-new-format-for-statcast-data-export-at-baseball-savant/)
...
For pitcher comparisons, pitchers are aggregated on a season, pitch type basis. These are the mean columns:
...
Our implementation will estimate $f(\cdot|\x)$ using the \texttt{kde2d} function in R. 
...
We develop novel performance metrics that are theoretically computed as expectations with respect to the synthetic spray chart distribution estimated in the last subsection. We estimate the expected number of singles, doubles, triples, homeruns that the batter hits versus the pitcher in a particular matchup. We also estimate the expected BABIP and xBsCON as additional summary measures. The metric xBsCON is best interpreted as slugging percentage conditional on balls put into play.
Our implementation will not estimate these expectations exactly, there is not enough historical batted ball data.

To theoretically estimate these quantities we first obtain XX years of batted ball data (\textbf{Charlie, how many years?}). We then estimate the proportion of batted balls that were either an out (O), single (1B), double (2B), triple (3B), or homerun (HR) at locations $\y$ on the baseball field. Denote this vector of estimated proportions at $\y$ as 
$
  \widehat \Pbf(\y) =   
...
$
  \text{BABIP} = \hat e_{\text{1B}}(\x) + \hat e_{\text{2B}}(\x) + \hat e_{\text{3B}}(\x)
$ 
