...
\begin{align*}
  ...
  &\qquad= \lambda f(\y|\x) +  \lambda \frac{\mu_2(K)\h'\text{diag}(\Hcal_f(\y|\x))\h}{2} 
    + \lambda_p \sum_{j=1}^J w_{p,j} \E \hat f_{\h_{p,j}}(\y|\x_{p,j},\x_{b,K+1}) \\
    &\qquad\qquad+ \lambda_b \sum_{k=1}^K w_{b,k} \E \hat f_{\h_{b,k}}(\y|\x_{p,J+1},\x_{b,k}) 
    + o(\|\h\|^2) \\
  &\qquad= \lambda f(\y|\x) +  \lambda \frac{\mu_2(K)\h'\text{diag}(\Hcal_f(\y|\x))\h}{2} 
  + o(\|\h\|^2) \\
    &\qquad\qquad+ \lambda_p \sum_{j=1}^J w_{p,j} f_{\h_{p,j}}(\y|\x_{p,j},\x_{b,K+1}) 
      + \lambda_p\sum_{j=1}^Jw_{p,j}
      \frac{\mu_2(K)\h_{p,j}'\text{diag}(\Hcal_f(\y|\x_{p,j},\x_{b,K+1}))\h_{p,j}}{2} \\
    &\qquad\qquad+ \lambda_b \sum_{k=1}^K w_{b,k} f_{\h_{b,k}}(\y|\x_{p,J+1},\x_{b,k}) 
      + \lambda_b\sum_{k=1}^K w_{b,k}
      \frac{\mu_2(K)\h_{b,k}'\text{diag}(\Hcal_f(\y|\x_{p,J+1},\x_{b,k}))\h_{b,k}}{2},
\end{align*}
and 
\begin{align*}
  &\Var(\hat g_{\lambdabf}(y|x)) = \Var\left(\lambda \hat f_\h(\y|\x) 
    + \lambda_p \hat f_{\text{sp}}(\y|\x_{b,K+1}) 
    + \lambda_b \hat f_{\text{sb}}(\y|\x_{p,J+1})\right) \\
  &= \lambda^2 \frac{R_{\x}(f)f(\y|\x)}{n\det(\Hbf)} + O\left(\frac{1}{n}\right)
    + \lambda_p^2 \sum_{j=1}^J w_{p,j}^2 \Var \hat f_{\h_{p,j}}(\y|\x_{p,j},\x_{b,K+1}) 
    + \lambda_b^2 \sum_{k=1}^K w_{b,k}^2 \Var \hat f_{\h_{b,k}}(\y|\x_{p,J+1},\x_{b,k}) \\
  &= \lambda^2 \frac{R_{\x}(f)f(\y|\x)}{n\det(\Hbf)} + O\left(\frac{1}{n}\right)
    + \lambda_p^2 \sum_{j=1}^J w_{p,j}^2
      \frac{R_{\tilde \x_{p,j}}(f)f(\y|\x_{p,j},\x_{b,K+1})}{n_{p,j}\det(\Hbf_{p,j})} 
    + \lambda_b^2 \sum_{k=1}^K w_{b,k}^2
      \frac{R_{\tilde\x_{b,k}}(f)f(\y|\x_{p,J+1},\x_{b,k})}{n_{b,k}\det(\Hbf_{b,k})},
\end{align*}
where $\tilde \x_{b,k} = (\x_{p,J+1}',\x_{b,k}')'$ and $\tilde \x_{p,j} = (\x_{p,j}',\x_{b,K+1}')'$. We will now choose the bandwidth parameters such that $\det(\Hbf_{p,j}) = \det(\Hbf)n/n_{\x_{p,j}}$ and $\det(\Hbf_{b,k}) = \det(\Hbf)n/n_{\x_{b,k}}$ in order to simplify our derivation for $\lambdabf$. With these specifications, we have that
\begin{align*}
  &\Var(\hat g_{\lambdabf}(y|x)) - \Var(\hat f_\h(\y|\x)) \\
  &=(\lambda^2 - 1)\frac{R_{\x}(f)f(\y|\x)}{n\det(\Hbf)} 
    + \lambda_p^2 \sum_{j=1}^J w_{p,j}^2
      \frac{R_{\tilde \x_{p,j}}(f)f(\y|\x_{p,j},\x_{b,K+1})}{n\det(\Hbf)} 
    + \lambda_b^2 \sum_{k=1}^K w_{b,k}^2 
      \frac{R_{\tilde\x_{b,k}}(f)f(\y|\x_{p,J+1},\x_{b,k})}{n\det(\Hbf)} \\
  &\leq \left(\lambda^2 + \lambda_p^2 \sum_{j=1}^Jw_{p,j}^2
    + \lambda_b^2 \sum_{k=1}^K w_{b,k}^2 - 1\right)\frac{R_{\x}(f)f(\y|\x)}{n\det(\Hbf)} \\
    &\qquad\qquad+ \lambda_p^2 \sum_{j=1}^J w_{p,j}^2
      \left(\frac{R_{\x}(f)\|\x-\tilde \x_{p,j}\|^2 
      + 2Lf(\y|\x)\|\x-\tilde \x_{p,j}\|^2 + 2L^2\|\x-\tilde \x_{p,j}\|^4}{n\det(\Hbf)}\right) \\
    &\qquad\qquad+ \lambda_b^2 \sum_{k=1}^K w_{b,k}^2
...
\begin{align*}
  &\text{Bias}(\hat g_{\lambdabf}(\y|\x),f(\y|\x))^2 = \left(\lambda f(\y|\x) - f(\y|\x) 
    ...
    &\qquad+ \lambda_p \sum_{j=1}^J w_{p,j} f_{\h_{p,j}}(\y|\x_{p,j},\x_{b,K+1}) 
      + \lambda_p\sum_{j=1}^Jw_{p,j}
      \frac{\mu_2(K)\h_{p,j}'\text{diag}(\Hcal_f(\y|\x_{p,j},\x_{b,K+1}))\h_{p,j}}{2} \\
    &\qquad+ \left.\lambda_b \sum_{k=1}^K w_{b,k} f_{\h_{b,k}}(\y|\x_{p,J+1},\x_{b,k}) 
      + \lambda_b\sum_{k=1}^K w_{b,k}
      \frac{\mu_2(K)\h_{b,k}'\text{diag}(\Hcal_f(\y|\x_{p,J+1},\x_{b,k}))\h_{b,k}}{2}\right)^2 
\end{align*}


%\begin{align*}
% &MSE(\hat g_{\lambdabf}(y|x)) - MSE(\hat f_\h(\y|\x)) 
%   = \Var(\hat g_{\lambdabf}(y|x)) - \Var(\hat f_\h(\y|\x)) 
%     + \text{Bias}(\hat g_{\lambdabf}(y|x)) - \text{Bias}(\hat f_\h(\y|\x)) \\
% &\qquad = (\lambda^2 - 1)\frac{R_{\x}(f)f(\y|\x)}{n\det(\Hbf)} + O\left(\frac{1}{n}\right)
%    + \lambda_p^2 \sum_{j=1}^J w_{p,j}^2
%      \frac{R_{\tilde \x_{p,j}}(f)f(\y|\x_{p,j},\x_{b,K+1})}{n\det(\Hbf)} \\
%    &\qquad\qquad+ \lambda_b^2 \sum_{k=1}^K w_{b,k}^2 
%      \frac{R_{\tilde\x_{b,k}}(f)f(\y|\x_{p,J+1},\x_{b,k})}{n\det(\Hbf)} 
